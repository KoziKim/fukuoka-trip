...
function openReview(id) {
  ...
  const photo = ph
    ? `<figure class="rvphoto"><img src="./photos/${p.id}.webp" alt="${esc(p.name)}" loading="lazy" width="480" height="360">
        <figcaption>${ph.kind === 'own' ? '📷 우리가 넣은 사진' : (ph.kind === 'dish' ? '대표 메뉴 예시 (이 가게 사진은 아님) · ' : '') + '사진: <a href="' + esc(ph.page) + '" target="_blank" rel="noopener">' + esc(ph.artist || 'Wikimedia Commons') + '</a>' + (ph.license ? ' · ' + esc(ph.license) : '')}</figcaption></figure>`
    : ''
